...
function main(): void {
  ...
  if (unresolved.length > 0) {
    console.log(
      `\n${unresolved.length} script reference(s) named by a prerendered page do not exist in ` +
        `.next. A chunk that cannot be read is a chunk that was not weighed and not scanned, so ` +
        `this run measured less than it reported:`,
    );
    for (const u of unresolved) console.log(`  ${u.ref} — referenced by ${u.page}`);
    failed = true;
  }
  ...
  // excluded zero-byte HTML, with its reason, before this loop runs. A page that
  // gets this far has bytes. A real public page with bytes and no script
  ...
  if (noScripts.length > 0) {
    console.log(
      `\n${noScripts.length} prerendered public page(s) named no script that this check could ` +
        `extract. Zero-byte HTML is excluded upstream, so these pages have bytes — and a page ` +
        `yielding no references was neither weighed nor scanned, whatever its ledger line says. ` +
        `Fix the extraction in scripts/check-public-bundle.ts:`,
    );
    for (const p of noScripts) console.log(`  ${p} — no script references extracted`);
    failed = true;
  }
  ...
  console.log(`\n  deps  scanned ${chunks.size} chunks across ${PAGES.length} pages`);
  for (const dep of BANNED_DEPS) {
