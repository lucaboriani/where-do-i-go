...
The design spec's §7 opens instead on `centresBbox`, the box enclosing every placed trip's
centre. That was ruled out before this task: with the two seeded trips the fit centres on
neither of them, somewhere in between, which reads as a broken camera rather than an overview.
Passing a `bbox` here as well would also mean two camera calls racing on `style.load`. So the
component makes no camera call at all, and adding one to it is the wrong place — the camera is
`useMapTrips`'s, whole.
